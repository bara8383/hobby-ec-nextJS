...
- `github_environment`: Terraform apply job の GitHub Environment 名。通常 `production`
...
bootstrap 実行ユーザーには、少なくとも次の権限が必要です。
...
`AccessDenied` で `iam:CreateOpenIDConnectProvider` が拒否された場合は、Terraform 側を変更せず、まず AWS 管理者に bootstrap 用 SSO ロールへの権限追加を依頼します。
...
### 3-1. 途中まで成功して 403 になった場合の再開方法

S3 bucket や DynamoDB table の作成後に IAM / OIDC 作成で失敗しても、ローカルの `infra/terraform/bootstrap/terraform.tfstate` は保持してください。
この state に部分成功したリソースが記録されているため、削除や初期化は不要です。

権限追加後は、同じディレクトリで次だけを再実行します。
...
この時点の plan では、主に次の IAM 3 リソースが作成対象になります。
