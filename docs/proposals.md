...
| P-031 | Concern | Infra | `infra/cdk/lib/chat-sse-stack.ts` はAPI Gateway REST + Lambdaを最小構成で追加したが、Response Streamingの有効化手順はリージョン/機能差分の影響を受ける可能性があり、現時点では実環境検証が未完了。【対応案】`curl -N` と CloudWatch Logs で chunk送信継続・15分切断・Abort時停止を確認する検証タスクを次PRで実施する。 | Infra | SSEがバッファリングされるとリアルタイム性が低下し、接続維持コストだけ増える恐れ。 | High | Open | Pending | - | - |
| P-032 | Concern | UX | Amazon/楽天のような「常時検索＋購買補助ヘッダー」と比較すると、現行の `components/layout/SiteHeader.tsx` はリンクナビ中心で、検索ボックス・カテゴリドロップダウン・カート件数・セール導線が未実装。トップ導線は `app/page.tsx` のヒーロー内検索に依存しており、下層回遊時の再検索効率が低い。【対応案】ヘッダーに常設検索コンポーネントを追加するか、既存 `/search` への入力UIを共通化するかを決定し、カテゴリ切替・カート件数表示・販促導線の採用範囲（常設/季節限定）を仕様化する。 | FE | 主要導線の再現性が不足し、商品発見率と回遊率が伸びにくい。SEO観点でも内部検索起点の行動データが分散しやすい。 | High | Open | Pending | - | - |
| P-033 | Concern | UX | Amazon/楽天/UNIQLOのPDP定番要素（レビュー平均、件数、FAQ/Q&A、比較表、安心訴求）に対し、`app/(store)/products/[slug]/page.tsx` は基本スペックとCTAのみで、購入判断の社会的証明が不足。さらに `components/ProductCard.tsx` でも一覧比較用の評価軸が価格以外に乏しい。【対応案】レビューとQ&Aを今すぐ実装するか、先行して「編集部コメント/利用シーン/ライセンス比較」の静的セクションを導入するかを決定し、最低限表示する評価指標（星評価・レビュー件数・用途タグ）を仕様化する。 | FE | 比較・意思決定に必要な情報が不足し、詳細離脱率やチャット依存度の上昇を招く。 | High | Open | Pending | - | - |
| P-034 | Improvement | UX | 楽天の特集ページやUNIQLOのシーズナルLPに相当する「販促用ランディング（セール/特集/ランキング）」が現行ルーティングに存在せず、`app/page.tsx` も静的セクション中心のため、キャンペーン訴求の差し替え運用が困難。【対応案】`/campaigns/[slug]` を新設するか既存ホームで吸収するかを決定し、掲載単位（クーポン、期間、対象カテゴリ）と運用主体（管理画面入力 or コード管理）を定義する。 | FE + SEO | 季節需要・特価訴求を機動的に展開できず、流入獲得とCV最適化の打ち手が限定される。 | Medium | Open | Pending | - | - |
| P-035 | Concern | CRM | Amazon/楽天の「閲覧履歴・お気に入り一覧・再訪導線」と比較すると、`components/product/FavoriteButton.tsx` はクライアント内トグルのみで永続化されず、`app/(store)/mypage/*` 配下にもウィッシュリスト/履歴画面が未実装。再訪時の検討再開導線が不足している。【対応案】お気に入りをサーバー保存するか（ユーザー単位DB）ローカル保存で開始するかを決定し、`/mypage/favorites` と「最近見た商品」導線の優先順位・保存期間・匿名ユーザー扱いを仕様化する。 | Fullstack | 再訪CVの底上げ施策が限定され、広告流入の再コンバージョン効率が低下する。 | Medium | Open | Pending | - | - |
