...
| 2026-02-15 | PR作成直後の本文検証自動化（廃止） | Improvement | ブラウザ版Codex環境では `gh` を利用できないため、ローカル専用スクリプト運用を廃止し、GitHub Actionsによる自動補完/ガードへ一本化。 | 環境依存の運用を減らし、PR本文品質担保をサーバー側で統一。 | ローカル専用スクリプトを削除し、Actions中心の運用を維持する。 | Done | Accepted | 2026-02-15 |
...
| 2026-02-15 | エンコーディング設定の明確化 | Improvement | リポジトリ直下に `.editorconfig` を追加し、UTF-8/LF/末尾改行を固定。併せて `.vscode/settings.json` で `files.encoding=utf8` を明示して BOM 付き保存（utf8bom）を避ける設定を追加。 | エディタ差異による文字化け・改行差分・BOM混入を防止し、レビュー時ノイズを削減。 | 現状設定を標準として運用し、必要なら将来 `*.md` など個別セクションを追加する。 | Open | Pending | - |
